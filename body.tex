...
\myframe{
  ...
  \begin{itemize}
    ...
    \item Voltaram ao \emph{mainstream}.
    \item São importantes na economia.
    \item São importantes na computação.
    \item São importantes na matemática.
    \item São importantes na pedagogia.
  \end{itemize}
}
...
\myframetop{
  ...
  \begin{itemize}
    \item Informação
      \only<2>{ \\
    {\bf Perfeita}: Jogo da velha, dama, xadrez, go,
      futebol, vôlei, tênis. \\
    {\bf Imperfeita}: Poker, dominó,
    \emph{Monopoly}, Starcraft. }
    \item Cooperativo
      \only<3>{ \\
        {\bf Sim}: RPGs, \emph{Forbidden Desert} \\
        {\bf Não}: Muitos outros
      }
    \item Simetria
      \only<4>{ \\
        {\bf Simétrico}: A maioria. \\
        {\bf Não}: \emph{Interpol}, Esconde-esconde.
      }
    \item Soma zero
      \only<5>{ \\
        {\bf Sim}: Muitos \\
    ...
    \item Simultaniedade (Real time vs Turn based)
      \only<6>{ \\
        {\bf Sim}: Grande parte dos esportes e jogos digitais. \\
        {\bf Não}: Grande parte dos jogos de tabuleiro.
      }
    \item Determinismo
      \only<7>{ \\
        {\bf Há sorte}: Jogos com dados, baralhos, etc.\\
        {\bf Não há}: Xadrez, Go, etc.
      }
  \end{itemize}
}

\section{Computador como jogador}
...
\myframe{
  ...
  \begin{itemize}
    \item Muita mais possibilidades
